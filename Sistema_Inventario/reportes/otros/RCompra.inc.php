	...
	function SetQuery($query){
		...
		echo "<table border=1 bordercolor=#84ADEE cellspacing=0 cellpadding=0 width=100%><tr  bgcolor=#610B0B>";
			...
			echo "	<body vlink=blue alink=blue link=blue>
					<th><font color=white face=verdana size=2 >N</th>
					<th><font color=white face=verdana size=2 >Producto</th>
					...
					<th><font color=white face=verdana size=2 >Descripci&oacute;n</th>
					<th><font color=white face=verdana size=2 >Fecha/compra</th>
					<th><font color=white face=verdana size=2 >Proveedor</th>
					<th><font color=white face=verdana size=2 >Cantidad</th>
					<th><font color=white face=verdana size=2 >Precio/compra</th>";
		...
		$calculo=0;
			while($data = mysql_fetch_row($result)){
				$consulta=mysql_fetch_array(mysql_query("Select * from Proveedores where codigo=$data[5]"));
				$inventario=mysql_fetch_array(mysql_query("Select * from Inventario where codigo_producto=$data[1]"));
				echo "<td><center><font face=verdana size=2>{$data[0]}</a></td>";
				echo "<td><center><font face=verdana size=2><font color=black>{$inventario[1]}</a></td>";
				echo "<td><center><font face=verdana size=2><font color=black>{$inventario[2]}</a></td>";
				echo "<td><center><font face=verdana size=2><font color=black>{$inventario[3]}</a></td>";
				echo "<td><center><font face=verdana size=2><font color=black>{$inventario[4]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$data[6]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$data[4]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$consulta[1]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$data[2]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$data[3]}</a></td>";
					
				echo "</tr>";
				
				$calculo = $data[2] * $data[3];
				$monto=$monto + $calculo;
			}
		...
		echo "<tr>
		<td colspan=9 align=right><font face=verdana size=2>Monto Total Compra:</a></td>
		<td><font face=verdana size=2><center>$monto</td>
		</table><br>";
