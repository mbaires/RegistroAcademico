...
class DataView{
	...
	function SetQuery($query){
		...
		if(mysql_num_rows($result) > 0){
			...
			$i=$i+1;
				$consulta_detalle=mysql_fetch_array(mysql_query("Select * from Ventas where N_factura=$data[0]"));
				$venta=$consulta_detalle[3] * $consulta_detalle[4];
				...
				echo "<td><center><font face=verdana size=2>{$consulta_detalle[3]}</a></td>";
				echo "<td><center><font face=verdana size=2>{$consulta_detalle[4]}</a></td>";
				echo "<td><center><font face=verdana size=2>$venta</a></td>";
				echo "</tr>";
				$suma=$suma + $venta;
			}
		
		echo "
		<tr><td colspan=5 align=right><font face=tahoma size=2><b>Monto Total Venta:
